...
import com.example.androidhw3.solarCalendar.DaysNamesEnum;
import com.example.androidhw3.solarCalendar.ShamsiMonthEnum;
...
public class CalendarActivity extends FragmentActivity {
	...
	/**
	 * reshape calendar to middle point
	 * set current item to middle
	 */
	private void reshapePagerAdapterToMiddle(){
		mCalendarPagerAdapter.reshapeCalendarToMiddle(mViewPager.getCurrentItem());
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mViewPager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
			...
			public void onPageSelected(int arg0) {
				...
				if(mCalendarPagerAdapter.isReshapeNeeded(mViewPager.getCurrentItem())){
					reshapePagerAdapterToMiddle();
				}
				CalendarActivity.CalendarFragment cf = (CalendarActivity.CalendarFragment)mCalendarPagerAdapter.getItem(mViewPager.getCurrentItem());
	...
	public boolean onOptionsItemSelected(MenuItem item) {
	    ...
	    switch (item.getItemId()) {
	        ...
	        case R.id.action_std2:
	        	Toast.makeText(getApplicationContext(), "90109993", Toast.LENGTH_SHORT).show();
	            return true;
	...
	public class CalendarPagerAdapter extends FragmentStatePagerAdapter{
		...
		public CalendarPagerAdapter(FragmentManager fm) {
			...
			calendarFragmentsRef = new CalendarFragment[MIDPOINT+MIDPOINTMARGIN+2];
			isDirty = false;
		}

		public boolean isReshapeNeeded(int position){
			return (position >= MIDPOINT+MIDPOINTMARGIN || position <= MIDPOINT-MIDPOINTMARGIN);
		}
		...
		public Fragment getItem(int position) {
			...
			if(calendarFragmentsRef[position] == null)
				calendarFragmentsRef[position] =  new CalendarFragment( getTodayCalendar(position) );
...
//			}
	...
	public static class CalendarFragment extends Fragment {
		...
		private void loadDataFromDB(){
			...
			//TODO load  [calendarTool.getDate(), costOfDay, incomeOfDay] from DB
//			Cost cost = null;
...
//			Cost cost = new Cost(calendarTool.getDate(), costOfDay, incomeOfDay);
			...
			//TODO save  [calendarTool.getDate(), costOfDay, incomeOfDay] in DB
		}
...
//			System.err.println(" $$ #ST INFORM DATA PERCENT : " + incomeOfDay + " -" + costOfDay + " @" + calendarTool+ " ::" + this );
...
//			System.err.println(" $$ #ST INFORM DATA PERCENT : " + incomeOfDay + " -" + costOfDay + " $" +calendarTool+ " ::" + this  + "   percent: " + percent);
			
			if(getView() != null){
//				System.err.println(" @@ MY getView : " + getView());
				System.err.println(getView().findViewById(R.id.calendar_circular_percent));
				CircularPercentView cpv = (CircularPercentView) getView().findViewById(R.id.calendar_circular_percent);
...
//			((TextView) rootView.findViewById(R.id.calendar_fragment_date_text)).setText(date_string);
//			System.err.println("create calendar page : " + date_string);

			costOfDay = incomeOfDay = 0;
			loadDataFromDB();
//			System.err.println(" $$ CAL_FRAG CONSTRUCTOR " + incomeOfDay + " -" + costOfDay + " $" + calendarTool+  " ::" + this);
			return rootView;
		}
		...
		public void onResume() {
			super.onResume();
			((Button) getView().findViewById(R.id.calendar_cost_input_ring_button)) .setOnClickListener(new onInputValueButtonClicked(getResources().getString(R.string.cost_of_day), true));
			((Button) getView().findViewById(R.id.calendar_income_input_ring_button)) .setOnClickListener(new onInputValueButtonClicked(getResources().getString(R.string.income_of_day), false));
		}
		...
		class onInputValueButtonClicked implements OnClickListener{
			...
			public onInputValueButtonClicked(String messageString, boolean isCost){
				submitedValue = 0;
				dialog = new InputValueDialog(messageString);
				this.isCost = isCost;
//				System.err.println("CREATE ONIVBC : " + messageString + " " + isCost);
			}
			        ...
			        builder.setTitle(messageStr)
			               ...
			               .setPositiveButton(R.string.submit, new DialogInterface.OnClickListener() {
			                   public void onClick(DialogInterface dialog, int id) {
			                	   System.out.println("! DIALOG SUBMITED : " + messageStr);

//			       				System.err.println("USE ONIVBC : " + messageStr + " " + isCost);
			                	   try{
			                		   submitedValue = Integer.parseInt(editText.getText().toString());
			                		   if(submitedValue >= 0){
			                			   if(isCost){
			                				   System.err.println(" ^^ COST OF DAY -> " + submitedValue);
			                				   setCostOfDay(submitedValue);
			                			   }
			                			   else{
			                				   System.err.println(" ^^ INCOME OF DAY -> " + submitedValue);
